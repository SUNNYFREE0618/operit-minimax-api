...
const minimaxApi = (function () {
    ...
    async function getApiKey() {
        const result = await Tools.SoftwareSettings.readEnvironmentVariable("MINIMAX_API_KEY");
        const envKey = result && result.value ? result.value.trim() : "";
    ...
    async function getApiBaseUrl() {
        const result = await Tools.SoftwareSettings.readEnvironmentVariable("MINIMAX_API_BASE_URL");
        const envUrl = result && result.value ? result.value.trim() : "";
        if (envUrl) {
    ...
    async function getVoiceId() {
        const result = await Tools.SoftwareSettings.readEnvironmentVariable("MINIMAX_VOICE_ID");
        const envVoiceId = result && result.value ? result.value.trim() : "";
        return envVoiceId;
    }
     ...
     *   - 否则 → 按 Base64 解码
     */
    function saveBase64Audio(audioData, savePath) {
        ...
        fos.write(bytes);
        fos.close();
        console.log("[minimax_api] 音频已写入: " + savePath + " (" + bytes.length + " bytes)");
        return savePath;
    }
    ...
    function httpPost(url, jsonBody, headers) {
        ...
        for (let i = 0; i < keys.length; i++) {
            requestBuilder.addHeader(keys[i], headers[keys[i]]);
        }
     ...
     */
    function buildMessages(messagesJson, systemPrompt) {
        let messages;
        try {
    ...
    async function chat_completion(params) {
        ...
        console.log("[minimax_api] 请求参数: temperature=" + (requestBody.temperature != null ? requestBody.temperature : '默认') + ", top_p=" + (requestBody.top_p != null ? requestBody.top_p : '默认') + ", max_tokens=" + (requestBody.max_completion_tokens != null ? requestBody.max_completion_tokens : '默认'));
    ...
    async function list_models() {
        ...
        for (let i = 0; i < modelKeys.length; i++) {
            const m = AVAILABLE_MODELS[modelKeys[i]];
            models.push({
                id: m.name,
                speed: m.speed,
    ...
    async function text_to_speech(params) {
        ...
        // 提取 Base64 音频数据
        const audioData = (resp.data && resp.data.audio) || resp.audio;
        ...
        // 诊断：打印 extra_info（如存在），帮助排查音频格式问题
        const extraInfo = (resp.data && resp.data.extra_info) || resp.extra_info;
        if (extraInfo) {
            console.log("[minimax_api] TTS extra_info: " + JSON.stringify(extraInfo));
        }
        console.log("[minimax_api] TTS audio 字符串长度: " + audioData.length + "，前60字符: " + audioData.substring(0, 60));
    ...
    return {
        chat_completion: function(params) { return wrapToolExecution(chat_completion, params); },
        list_models: function() { return wrapToolExecution(list_models, undefined); },
        text_to_speech: function(params) { return wrapToolExecution(text_to_speech, params); }
    };
})();
